...
def RM_integration(n_e: np.ndarray, B_field: np.ndarray, pixsize: float, axis: int) -> np.ndarray:
    """
...
def RM_halfway(n_e: np.ndarray, B_field: np.ndarray, pixsize: float, axis: int) -> np.ndarray:
    """
    Calculate Rotation measure by 'integrating' over a certain axis.
...
def xvector(N: int, ndim: int, pixsize: float, ftype: Callable, subcube: bool = False) -> np.ndarray:
    """
...
def xvector_length(N: int, ndim: int, pixsize: float, ftype: Callable, subcube: bool = False) -> np.ndarray:
    """
...
def cube_from_subcube(subcube: np.ndarray, cubeshape: int, ftype: Callable) -> np.ndarray:
    """
...
def magnetic_field_crossproduct(
    ...
    if Neven == 0:
        ...
        fourier_B_field[N // 2, :N // 2, N // 2] = np.cross(
            1j * kvec[N // 2, :N // 2, N // 2], field[N // 2, :N // 2, N // 2], axis=-1
        )       
        ...
        raise NotImplementedError("Only implemented for even N, please request an even amount of pixels.")
...
def gaussian_random_field3D(
    ...
    run_fftw = pyfftw.builders.rfftn(
        np.random.normal(size=(N, N, N)).astype(ftype),
...
def calculate_vectorpotential(
    ...
    field: np.ndarray = np.zeros((N, N, N // 2 + 1, 3), dtype=ftype) + 1j * np.zeros((N, N, N // 2 + 1, 3), dtype=ftype)
...
def normalise_Bfield(
    ...
    average_profile = np.mean(B_field_amplitude)  # B field should have no radial dependence yet
...
def shell_averaged_power_spectrum2D(field: np.ndarray, multiply_volume: bool = False) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def plotRMimage(RMimage: np.ndarray, pixsize: float, title: Optional[str] = '') -> None:
    N = len(RMimage)
...
def convolve_with_beam(images: List[np.ndarray], FWHM: float, pixsize: float = 1.0) -> List[np.ndarray]:
    """
...
def normalise_ne_field(
    ...
    average_profile: float = np.mean(ne_fluct)  # ne field should have no radial dependence yet
                                        
    ne_3d: np.ndarray = ne_funct(xvec_length)  # (N, N, N) for subcube=False or (N//2+1, N//2+1, N//2+1) for subcube=True
