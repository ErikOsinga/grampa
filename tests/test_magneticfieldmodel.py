...
class TestMagneticFieldModel(unittest.TestCase):
    ...
    def test_Bfield_generation(self):
        ...
        self.assertFalse(np.isnan(self.model.B_field_norm).any(), "B-field array should not contain NaNs.")
        self.assertFalse(np.isnan(self.model.RMimage).any(), "RM image should not contain NaNs.")
        self.assertTrue(np.all(self.model.Polint <= 1) & np.all(self.model.Polint >= 0) , "Polarization intensity should be between 0 and 1.")
        self.assertFalse(np.any(np.abs(self.model.RMimage) > 1e5), "Faraday rotation measure should not exceed 1e5 for a small cube.")
        ...
        self.assertEqual(len(all_r), len(profile), "Radius and profile arrays must be of equal length.")
        self.assertGreater(len(all_r), 0, "Radius array should not be empty.")
        self.assertTrue(np.all(profile >= 0), "Magnetic field amplitude should not be negative.")
        self.assertTrue(np.all(density >= 0), "Density values should not be negative.")

        expect_B = ((density/density[0])**0.5)*self.model.B0
        self.assertTrue(np.allclose(np.mean(expect_B/profile), 1, rtol=2), "B-field does not follow density profile.")

        # Check if B-field follows power spectrum reasonably well
        if self.model.lambdamax < self.model.N*self.model.pixsize/2:
            raise NotImplementedError("Did not implement test for power spectrum for lambdamax not equal to maximum scale")
        k_values, Pk_values, theoretical = mutils.plot_B_field_powerspectrum(self.model.B_field_norm, self.model.xi, self.model.lambdamax
                                                                             , savefig='./test_Bfield_powerspectrum.png', show=False)
        ...
        slope *= -1
        self.assertTrue(np.isclose(slope, self.model.xi-2, rtol=0.1), "Power spectrum slope does not match expected value to within 10%.")
